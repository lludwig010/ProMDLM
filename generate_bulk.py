...
for i in range(nb_generated_sequences):
    ...
    df = pd.concat([df, pd.DataFrame([{'Sequences': decode.replace(" ","")}])], ignore_index=True)

df.to_csv(os.path.join(config.paths.output_dir, config.paths.csv_save_name), index=False)
