...
def calculate_sequence_entropy(input_csv):
    ...
    df.to_csv(
        os.path.join(output_dir, file_name_without_extension + "_results_entropy.csv"),
        index=False,
    )
    print(
        f"BLASTP analysis completed. Results saved to {os.path.join(output_dir, file_name_without_extension+ "_results_entropy.csv")}"
    )
