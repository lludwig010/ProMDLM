...
def main():
    ...
    if db == None:
        print(
            "Cannot find {} {} BLAST database".format(
                "protein" if ext == "p" else "nucleotide", args.db
            ),
            file=sys.stderr,
        )
        return 1
...
def create_arg_parser():
    ...
    parser.add_argument(
        "-dbtype", help="Molecule type", required=True, choices=["prot", "nucl"]
    )
    parser.add_argument(
        "-dry-run", action="store_true", help="Do not delete any files, just list them"
    )
    parser.add_argument("-version", action="version", version="%(prog)s " + VERSION)
    parser.add_argument(
        "-verbose", action="count", default=0, help="Increase output verbosity"
    )
    return parser
