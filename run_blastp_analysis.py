...
def run_blastp_analysis(input_csv, db_name):
    ...
    csv_to_fasta(
        input_csv, fasta_filename, sequence_col="sequence"
    )  # the fasta id is the index of the csv file
    ...
    merged_df.to_csv(
        os.path.join(output_dir, file_name_without_extension + "_results_blastn.csv"),
        index=False,
    )
    os.remove(fasta_filename)
    os.remove(blastp_output)
    print(
        f"BLASTP analysis completed. Results saved to {os.path.join(output_dir, input_csv)}"
    )


if __name__ == "__main__":
    input_csv = [
        "generated_sequences/generated_sequences_fulldiff.csv",
