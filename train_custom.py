    ...
    def train_loop_fullDiff(self):
        ...
        for self.epoch in range(self.epochs):
            ...
            for i, batched_sequences_tokenized in enumerate(self.train_loader):
                ...
                batched_sequences_tokenized = batched_sequences_tokenized[:, :].to(
                    self.device
                )  # remove eos?
                ...
                batch_masks = noise_schedule(
                    self.max_timesteps, t, size_to_mask, self.seq_len
                )
                masked_batch_seq, batch_masks = apply_noise(
                    batch_masks, batched_sequences_tokenized, t
                )
                batch_pred_tokens = self.model(masked_batch_seq)
                batch_loss = scheduler_loss_fn(
                    batch_pred_tokens,
                    batched_sequences_tokenized,
                    masked_batch_seq,
                    self.vocab_size,
                )
                ...
                # handle exploding gradient
                    ...
                    self.logger.warning(
                        f"GRADIENTS EXPLODED FOR BATCH {i} TIMESTEP {t}, WE DO NOT UPDATE THE WEIGHTS."
                    )
                    percent_masked = (
                        torch.sum(batch_masks == 1).item() / batch_masks.numel()
                    )
            ...
            if self.validation:
                ...
                for i, batched_sequences_tokenized in enumerate(self.val_loader):
                    ...
                    batch_pred_tokens = self.model(masked_batch_seq)
                    val_batch_loss = scheduler_loss_fn(
                        batch_pred_tokens,
                        batched_sequences_tokenized,
                        masked_batch_seq,
                        self.vocab_size,
    ...
    def train_increment_diffusion(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batched_sequences_tokenized in enumerate(self.train_loader):
                ...
                timestep = (
                    (epoch / self.epochs) + (i / nb_batch_train) * (1 / self.epochs)
                ) * self.max_timesteps
                ...
                max_timestep = min(
                    self.max_timesteps, timestep + 0.1 * self.max_timesteps
                )
